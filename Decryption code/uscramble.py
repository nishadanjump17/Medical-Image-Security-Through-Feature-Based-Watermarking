...
uscramble_img=scramble(img[:64,:64],x,64,64)
(thresh, im_bw1) = cv2.threshold(uscramble_img, 128, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
(thresh, im_bw2) = cv2.threshold(uscramble_img, 128, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
(thresh, im_bw3) = cv2.threshold(uscramble_img, 128, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)


cv2.imwrite('Damaged Image.jpg', im_bw1)
...
for i in range(64):
                for j in range(64):
                        im_bw2[i,j]=int(hashcode[i])^int(im_bw1[i,j])
